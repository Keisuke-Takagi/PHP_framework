...
if($_SERVER['REQUEST_METHOD'] == "GET"){
  ...
  foreach ($rows as $row) {
    $instance->img_search($row);
  }
}
class SearchImg {
  public function img_search($row){
    // header('Content-Type:' . $row['image_type']);
    // $a = base64_encode($row["raw_data"]);
    echo '<img src="data:image/jpeg;base64,' .$row["raw_data"] .'"/>';
  }
}
//   $database = new Database();
// $dbh = $database->open();
...
// var_dump($image_name);
?>
<!-- <img src="" width="100px" height="auto" class="mr-3"> -->
