...
//   <a href="registration.php">
  class Insertview extends Baseview{
    ...
    public function php_error_print($error){
      ...
      }elseif($error["count"] == 2){
        return '<error><a href="../login/login" class="error">'. $error['email'] .'既にそのメールアドレスは登録されています<br>ログインへ';
      }elseif($error == 3){
        return '<error><p class="error">パスワードは半角英数字をそれぞれ一文字以上含んだ5文字以上で設定して下さい';
      }elseif($error == 4){
        return '<error><p class="error">入力されたemailが不正です<br>パスワードは半角英数字をそれぞれ一文字以上含んだ5文字以上で設定して下さい</p>';
      }
    }
      // echo __FILE__;
      ...
      //   $count = 0;
      //   echo "<br>" . dirname(dirname(dirname(__FILE__)));
